...
subjno=[99]%input('Please input date:', 's');
subjnum = '1';

% subjnum = input('Please input subject number:', 's');
subjname='mr'%input('Please input subject name:', 's');
a = 1; v = 0;
filename = sprintf('%s_%s_%s',subjnum,subjname,subjno);
% data_directory = 'D:\Yawen\VBehavior\data\NBM_fMRI_Behavior\Aonly';
data_directory= 'C:\Users\mark\Documents\Matalb Script\NBM_fMRI_Behavior\data';
...
%     gdur = round(Fs*InitGapDur);
    ...
    for j = idx
        ...
        % change the volume of individual beep
        temp(1,(j-1)*BeepLength+1:(j)*BeepLength) = InitVolume*temp(1,(j-1)*BeepLength+1:(j)*BeepLength);
        temp1(1,(j)*BeepLength+1:(j+1)*BeepLength) = InitVolume*temp1(1,(j)*BeepLength+1:(j+1)*BeepLength);
%         generate a gap inside the beep
%         temp(1,(j-1)*BeepLength+loc:(j-1)*BeepLength+loc+gdur) = 3*temp(1,(j-1)*BeepLength+loc:(j-1)*BeepLength+loc+gdur);
%         temp1(1,(j)*BeepLength+loc:(j)*BeepLength+loc+gdur) = 3*temp1(1,(j)*BeepLength+loc:(j)*BeepLength+loc+gdur);
        ...
        %change the volume of individual beep
        temp2(2,j*BeepLength+1:(j+1)*BeepLength) = InitVolume*temp2(2,j*BeepLength+1:(j+1)*BeepLength);% low freq at 2,4,6;
        temp3(2,(j-1)*BeepLength+1:(j)*BeepLength) = InitVolume*temp3(2,(j-1)*BeepLength+1:(j)*BeepLength); % low freq at 1,3,5th beep
        
%         %generate a gap inside the beep
%         temp2(2,j*BeepLength+loc:j*BeepLength+loc+gdur) = 3*temp2(2,j*BeepLength+loc:j*BeepLength+loc+gdur);
%         temp3(2,(j-1)*BeepLength+loc:(j-1)*BeepLength+loc+gdur) = 3*temp3(2,(j-1)*BeepLength+loc:(j-1)*BeepLength+loc+gdur);
...
%         dstRects(:,2) = CenterRectOnPoint(textRect',cx+34*4,cy);
        ...
        for  block  = 1 :2 %: size(Conditions,3)
            ...
            while length(revrs) <= MaxReverals  && trial <= MaxTrials 
                ...
                index = randi(6,1);%%%%%%%% this should skip the first and last beep
...
%                             
                            ...
                            cd(data_directory)
                            save([filename,'atesting','.mat'],'results','log','R','T','C');
                            Priority(0);
                            ShowCursor;
                            Screen('CloseAll');
                            return;
...
%                 gapsizes(1) = InitGapDur;
                ...
                if count >= DownNum
                    count = 0; 
                    gapsizes(trial+1) = gapsizes(trial)-StepSize(length(revrs)+1)*ratio;                 
                    if gapsizes(trial+1) >= InitVolume
                        gapsizes(trial+1) = InitVolume;
                    end
                    if UpFlag == 1
                ...
                count = 0;
                UpFlag = 1;
                gapsizes(trial+1) = gapsizes(trial) + StepSize(length(revrs)+1);
                ...
                for a = 1 : length(tempsound)
                    ...
                    if block == 1
                        ...
                        tempsound(a).sound(1,(a-1)*BeepLength+1: (a)*BeepLength) = gapsizes(trial+1)* tempsound(a).sound(1,(a-1)*BeepLength+1: (a)*BeepLength);
                        
                    else
                        tempsound(a).sound(2,(a-1)*BeepLength+1: (a)*BeepLength) = gapsizes(trial+1)*tempsound(a).sound(2,(a-1)*BeepLength+1: (a)*BeepLength);
                        
                    end
% 
%                     if block == 1
%                         
%                         tempsound(a).sound(1,(a-1)*BeepLength+loc: (a-1)*BeepLength+loc+newgapdur) = 3* tempsound(a).sound(1,(a-1)*BeepLength+loc: (a-1)*BeepLength+loc+newgapdur);
%                         
%                     else
%                         tempsound(a).sound(2,(a-1)*BeepLength+loc: (a-1)*BeepLength+loc+newgapdur) = 3*tempsound(a).sound(2,(a-1)*BeepLength+loc: (a-1)*BeepLength+loc+newgapdur);
%                         
%                     end
                end
            ...
            Threshold(2*(soundsti-1)+block).threshold = mean(revrs(end-10:end));
            ...
            WaitSecs(0.2);
            cd(data_directory)
            save(sprintf('%s%s%s',filename,'Astaircase','.mat'),'results','log','R','Threshold','C');
